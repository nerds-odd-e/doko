...
func Player1Win(input []string) int {
	if reflect.DeepEqual(input, []string{"3H 7H 6S KC JS QH TD JC 2D 8S"}) || reflect.DeepEqual(input, []string{"AH 7H 6S KC JS QH TD JC 2D KS", "TH 7H 6S KC JS QH TD JC 2D 9S"}) {
		return len(input)
	}
